...
class DrawBuf
 {
  ...
  public:
   ...
   void erase(Pane pane,BlenderType auto blender) { buf.block_safe(map(pane),blender); }

   void fill(Pane pane,RasterFillType<DesktopColor> auto fill) { buf.fill_safe(map(pane),fill); }
