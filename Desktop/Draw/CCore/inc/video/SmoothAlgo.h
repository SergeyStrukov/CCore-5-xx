...
template <class R>
concept MPointRangeType = Algon::RangeType<R> && requires(R &obj,ulen ind)
 {
  Ground<MPoint>( *obj );

  obj+=ind;

  Ground<MPoint>( obj[ind] );
 } ;

/* concept MPointMapType<Map,R> */

template <class Map,class R>
concept MPointMapType = Algon::RangeType<R> && requires(R &obj,Map &map,ulen ind)
 {
  Ground<MPoint>( map(*obj) );

  obj+=ind;

  Ground<MPoint>( map(obj[ind]) );
 } ;

/* concept PlotType<Plot> */

template <class Plot>
concept PlotType = NothrowCopyCtorType<Plot> && requires(Plot &obj,MPoint p,unsigned alpha)
 {
  Ground<Pane>( obj.getClip() );
...
void PutWithoutFirst(CursorOverType<MPoint> auto r,FuncArgType<MPoint> auto &func)
 {
  for(++r; +r ;++r) func(*r);
 }
...
struct SolidBox
 {
  ...
  template <class R>
  SolidBox(R dots,MPointMapType<R> auto map)
   {
    if( +dots )
      {
       MPoint a=map(*dots);
   ...
   class Full : NoCopy
    {
     ...
     private:
      ...
      void push(MCoord x,PlotType auto &plot)
       {
        if( count )
          {
           if( x<base.x ) return;
      ...
      void pixel(MCoord x,PlotType auto &plot)
       {
...
class SolidDriver : NoCopy
 {
   ...
   struct Line
    {
     ...
     bool operator == (const Line &obj) const { return x_bottom==obj.x_bottom && x_top==obj.x_top ; }

     CmpResult operator <=> (const Line &obj) const
      {
       return AlphaCmp(x_bottom,obj.x_bottom,x_top,obj.x_top);
      }
    };
  ...
  public:
   ...
   template <class R>
   SolidDriver(R dots,MPointMapType<R> auto map)
   ...
   explicit SolidDriver(MPointRangeType auto dots) : SolidDriver(dots, [] (MPoint point) { return point; } ) {}
