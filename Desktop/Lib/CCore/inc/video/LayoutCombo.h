...
template <class R>
concept LayRangeType = NothrowCopyCtorType<R> && requires(R r)
 {
  typename R::AdapterType;

  Ground<ulen>( r.getLen() );
...
template <class L>
concept LaySpaceType = requires(const L &lay,Coord space)
 {
  Ground<Coord>( lay.getSpace(space) );
 } ;
