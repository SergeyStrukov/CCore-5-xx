...
template <class T>
concept ConfigType = OneOfTypes<T,unsigned,Coord,Fraction,VColor,Clr,Point,String,FontParam,bool,Ratio> ;
...
class ConfigMap : NoCopy
 {
  ...
  private:
   ...
   void add(StrLen name,const ConfigType auto &value);
  ...
  public:
   ...
   void sync(StrLen name,ConfigType auto &ret)
    {
   ...
   void update(StrLen name,const ConfigType auto &value)
    {
     ConfigItem &item=find_or_add(name);
