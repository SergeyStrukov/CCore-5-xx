...
void BookMap::prepare(StrLen book_file_name)
 {
  if( SuffixExt(book_file_name).equal(".vol"_c) )
    {
     file=VolumeAbstFileToRead(book_file_name);
     file_name="/main.book.ddl"_c;
    }
  else
    {
     file=DefAbstFileToRead();
  ...
  try
    {
     ...
     using FileType = PretextFileToMem<AbstFileToMem,Pretext::Object> ;

     DDL::FileEngine<FileName,FileType,AbstFileToRead> engine(file,eout);
