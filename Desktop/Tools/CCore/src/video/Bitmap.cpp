...
void BitmapData::loadBitmap(StrLen file_name)
 {
  GetBinaryFile file(file_name);

  loadBitmap(file);
 }

void BitmapData::loadBitmap(const AbstFileToRead &binfile,StrLen file_name)
 {
  GetAbstFile file(binfile,file_name);
...
void BitmapData::saveBitmap(StrLen file_name) const
 {
  PutBinaryFile file(file_name,Open_ToWrite|Open_AutoDelete);
...
void BitmapData::loadZipmap(const AbstFileToRead &binfile,StrLen file_name)
 {
  GetAbstFile file(binfile,file_name);
...
void Bitmap::fill(FuncType<uint32> auto next)
 {
...
void Bitmap::load(const AbstFileToRead &binfile,StrLen file_name)
 {
  if( file_name.hasSuffix(".zipmap"_c) )
    {
     BitmapData data;
...
Bitmap::Bitmap(const AbstFileToRead &file,StrLen dir,StrLen file_name)
 {
  MakeFileName temp(dir,file_name);
