...
struct Fixed
 {
  ...
  static constexpr ulen SaveLoadLen = SaveLenCounter<uint8,uint16,uint32,uint64>::SaveLoadLen ;
...
class TestGetDev
 {
  ...
  public:
   ...
   static auto GetName() { return typeid(T).name(); }

   template <class Custom,class ... TT>
   void use_buf(TT & ... tt)
    {
     Printf(Con,"use_buf() #; #;\n",GetName<TT...>(),sizeof ... (TT));

     dev.use_buf<Custom>(tt...);
    }
...
void test4()
 {
  ...
  Printf(Con,"const len = #;\n",SaveLenCounter<AltFixed>::SaveLoadLen);
