...
template <class Ran>
concept SortRanType = RanType<Ran> && OpLessType< Meta::PtrObjType<Ran> > ;

/* concept SortContextType<Ctx,Ran> */

template <class Ctx,class Ran>
concept SortContextType = requires(Ctx ctx,Ran a,Ran b)
 {
  Ground<bool>( ctx.less(a,b) );
...
struct ShortSort
 {
  ...
  static bool Sort(Ran a,ULenType auto len,Ctx ctx)
   {
    switch( len )
      {
  ...
  static bool Sort(Ran a,ULenType auto len) requires ( DefaultCtorType<Ctx> )
   {
    switch( len )
      {
