...
concept ObjInfoType3 = !Lock || requires(Info info)
 {
  Ground<ObjBase *>( info.getObj() );
 } ;

template <class Info,class Extra>
concept ObjInfoType2 = requires(ObjBase *obj,StrLen name,Extra ext)
 {
  Extra(obj);
...
template <class Info>
concept ObjInfoType = requires()
 {
  typename Info::Extra;

  Ground<bool>( Info::Lock );
...
class ObjHost : NoCopy
 {
  ...
  private:
   ...
   auto apply(FuncInitArgType<ObjBase &> auto func_init) // locked operation
    {
