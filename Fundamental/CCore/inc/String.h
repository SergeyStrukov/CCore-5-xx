...
class String
 {
  ...
  public:
   ...
   String(DoBuildType,BuilderType<char> auto builder) : data(DoBuild,builder) {}
   ...
   void print(PrinterType auto &obj,PrintOptType opt) const
    {
   ...
   void objSwap(String &obj) noexcept
    {
     Swap(data,obj.data);
    }

   explicit String(ToMoveCtor<String> obj) noexcept
...
concept StrLenCastableType = requires(Meta::ToConst<T> &obj)
 {
  StrLenCast::Cast(obj);
 } ;
...
String operator + (const String &a,const StrLenCastableType auto &t) { return StringSum(a,t); }
