...
concept FuncArgCountType = ( ArgCount<Func>::Ret == Count ) ;
     ...
     class Base : public MemBase_nocopy
      {
       ...
       public:
        ...
        bool decRef() noexcept { return !--ref_count; }
...
class Contour : public NoCopyBase<Formular>
 {
   ...
   struct Key
    {
     ...
     bool operator == (const Key &obj) const noexcept { return hash==obj.hash && Range(name).equal(Range(obj.name)) ; }
    };

   friend CmpResult Cmp(StrKey a,const Key &b)
    {
     if( CmpResult ret=Cmp(a.hash,b.hash) ) return ret;
  ...
  private:
   ...
   struct Bind
    {
     ...
     void operator () (AnyType auto s) { if( !s.rex ) func(label,selected,s); }
   ...
   struct BindRef
    {
     ...
     void operator () (AnyType auto &s) { func(label,obj,s); }
   ...
   static Real ToReal(AnyType auto);

   static void CopyFlags(Label &label,AnyType auto);
