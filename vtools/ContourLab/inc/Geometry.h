...
concept ArgCursorType = requires(R &obj,Meta::ToConst<R> &cobj)
 {
 ...
 } /* && NothrowCopyableType<R> && NullableType<R> */ ; // BUG in GCC ?
...
struct Geometry
 {
  ...
  struct Path
   {
    ...
    Path(ArgCursorType<Step> auto cur)
     : rex(RealOk)
     {
      for(; +cur ;++cur)
        {
  ...
  struct Loop
   {
    ...
    Loop(ArgCursorType<Step> auto cur)
     : rex(RealOk)
     {
      for(; +cur ;++cur)
        {
  ...
  template <class Cur> requires ArgCursorType<Cur,Point>
  static Step StepOf(Cur cur) { return Step(cur); }

  template <class Cur> requires ArgCursorType<Cur,Point> || ArgCursorType<Cur,Step>
  static Path PathOf(Cur cur) { return Path(cur); }

  template <class Cur> requires ArgCursorType<Cur,Point> || ArgCursorType<Cur,Step>
  static Loop LoopOf(Cur cur) { return Loop(cur); }
