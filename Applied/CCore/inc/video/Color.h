...
concept BlenderType = NothrowCopyableType<Blender> && requires(Blender &obj,Clr c)
 {
  Ground<Clr>( obj.blendR(c) );
  Ground<Clr>( obj.blendG(c) );
  Ground<Clr>( obj.blendB(c) );
 } ;
...
class RawColor16
 {
  ...
  public:
   ...
   static void BlendTo(BlenderType auto blender,Raw dst[RawCount])
    {
     Raw raw=dst[0];
   ...
   void blend(BlenderType auto blender) { BlendTo(blender,color); }
...
concept RawColorType2 = requires(RawColor &obj,const RawColor &cobj,Raw *dst,const Raw *src,Blender blender)
 {
  cobj.copyTo(dst);
...
concept RawColorType = NothrowCopyableType<RawColor> && requires(RawColor &obj,const RawColor &cobj,Clr alpha,VColor vc,Clr r,Clr g,Clr b,Blender blender)
 {
  typename RawColor::Raw;

  Ground<unsigned>( RawColor::RawCount );

  RawColor();

  RawColor(vc);

  Ground<Clr>( cobj.getR() );

  Ground<Clr>( cobj.getG() );

  Ground<Clr>( cobj.getB() );

  Ground<VColor>( cobj.get() );
