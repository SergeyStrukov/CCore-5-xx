...
template <class T>
concept HashFuncType = NothrowDtorType<T> && NothrowCopyableType<T> && requires(T &func,const uint8 *data,ulen len,uint8 *digest)
 {
  Ground<ulen>( T::DigestLen );
...
template <class T>
concept HashType = NothrowDtorType<T> && requires(T &obj,const uint8 *ptr,ulen len,PtrLen<const uint8> data,uint8 *digest)
 {
  Ground<ulen>( T::DigestLen );
  Ground<ulen>( T::BlockLen );
...
template <class T>
concept KeyedHashType = HashType<T> && requires(T &obj,const uint8 *ptr,ulen len,PtrLen<const uint8> data)
 {
  T(ptr,len);
