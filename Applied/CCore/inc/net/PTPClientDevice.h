...
struct Tailed
 {
  ...
  void load(ExtraLoadDevType auto &dev)
   {
...
concept ExtType2 = requires(Ext &obj,const OutputType &output)
 {
  obj.done(output);
 } ;

template <class Ext>
concept ExtType = requires(Ext &obj,Packet<uint8,Ext> packet,TransResult result,ErrorIdType error_id)
 {
  Ground<ServiceIdType>( Ext::ServiceId );
  Ground<FunctionIdType>( Ext::FunctionId );

  Ground<TransResult Ext::*>( &Ext::result );
  Ground<ErrorIdType Ext::*>( &Ext::error_id );

  Ground<MoveFlagType>( Ext::GetMoveFlag(packet) );

  Ground<bool>( obj.isOk() );
