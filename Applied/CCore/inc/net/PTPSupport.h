...
template <class T>
concept TimeoutType = OneOfTypes<T,MSec,TimeScope> ;

/* concept FillType<T> */

template <class T>
concept FillType = requires(T &obj,PtrLen<uint8> buf)
 {
  Ground<ulen>( obj.getLen() );
...
struct TransStatus
 {
  ...
  bool guard(ExceptionType ex,ClientDevice *client,PacketSet<uint8> &pset,const char *name,TimeoutType auto timeout,ServiceIdType service_id,FunctionIdType function_id);

  template <ExtType Ext>
  bool guard(ExceptionType ex,ClientDevice *client,PacketSet<uint8> &pset,const char *name,TimeoutType auto timeout)
   {
...
class Support : public Funchor_nocopy
 {
  ...
  public:
   ...
   template <class ... SS>
   bool perform(TimeoutType auto timeout,SS ... ss)
    {
   ...
   template <class ... SS>
   bool perform_fill(TimeoutType auto timeout,FillType auto &fill,SS ... ss)
    {
     Packet<uint8> packet=pset.get(timeout);
   ...
   bool guard(ExceptionType ex,const char *name,TimeoutType auto timeout)
    {
...
class Support_Exist : NoCopy
 {
  ...
  public:
   ...
   bool perform(TimeoutType auto timeout,ServiceIdType service_id,FunctionIdType function_id)
    {
...
class Support_ErrorDesc : NoCopy
 {
  ...
  public:
   ...
   bool perform(TimeoutType auto timeout,ServiceIdType service_id,FunctionIdType function_id,ErrorIdType error_id)
    {
...
bool TransStatus::guard(ExceptionType ex,ClientDevice *client,PacketSet<uint8> &pset,const char *name,TimeoutType auto timeout,ServiceIdType service_id,FunctionIdType function_id)
 {
  if( ok ) return true;
...
class Support_Seed : NoCopy
 {
  ...
  public:
   ...
   bool perform(TimeoutType auto timeout)
    {
     return support.perform(timeout);
    }

   void perform_guarded(TimeoutType auto timeout)
    {
...
class Support_Echo : NoCopy
 {
  ...
  public:
   ...
   bool perform(TimeoutType auto timeout,uint32 delay_msec,FillType auto &fill)
    {
     return support.perform_fill(timeout,fill,delay_msec);
    }

   void perform_guarded(TimeoutType auto timeout,uint32 delay_msec,FillType auto &fill)
    {
