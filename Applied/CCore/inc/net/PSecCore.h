...
struct LifeLim
 {
  ...
  void use(UIntType auto use_count) { PosDec(utl,use_count); }

  void tick(UIntType auto dtime) { PosDec(ttl,dtime); }
 };
...
class KeySet : NoCopy
 {
   ...
   struct Key : NoCopy
    {
     ...
     void use(UIntType auto use_count)
      {
       if( state<KeyDead ) life_lim.use(use_count);
      }

     void tick(UIntType auto dtime)
      {
   ...
   struct Repeat
    {
     ...
     void tick(UIntType auto dtime) { PosDec(timeout,dtime); }
    };
  ...
  private:
   ...
   static ulen GetIndex(KeyIndex key_index) { return ulen( key_index&0x3FFFu ); }
