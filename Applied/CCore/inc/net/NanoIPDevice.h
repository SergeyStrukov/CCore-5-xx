...
class TxPrepareList : public Funchor_nocopy
 {
  ...
  public:
   ...
   bool put(PacketType auto packet,PacketFunction function) { return put(GetPacketHeader(packet),function); }
...
class ICMPEchoEndpoint : NoCopy
 {
   static constexpr ulen DataLen = 32 ;
...
class NanoIPDevice : public ObjBase , public Funchor , EthProc
 {
  ...
  public:
   ...
   bool send_prepare(PacketType auto packet,PacketFunction function)
    {
   ...
   void send_prepare_or_complete(PacketType auto packet,PacketFunction function)
    {
     if( prepare_list.put(packet,function) )
       {
        eth->signalOutbound();
