...
concept TOCType2 = requires(BuilderType &builder)
 {
  Ground<ObjectType *>( builder.create() );
 } ;

template <class TOC>
concept TOCType = UnidType<TOC> && requires()
 {
...
concept FullTOCType2 = requires(ObjectType &obj)
 {
  { obj.destroy() } noexcept;
 } ;

template <class TOC>
concept FullTOCType = TOCType<TOC> && requires()
 {
   ...
   struct Slot
    {
     ...
     void destroy(ulen slot_id) noexcept
      {
