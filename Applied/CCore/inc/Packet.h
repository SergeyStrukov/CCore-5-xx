...
class PacketHeader : NoCopy
 {
  ...
  public:
   ...
   T * pushExt(SS && ... ss) requires ( NothrowDtorType<T> && ConstructibleType<T,SS...> )
    {
     static_assert( Align(sizeof (T)) <= MaxPacketExtLen ,"CCore::PacketHeader::pushExt<T>(...) : T is too large");
...
template <class P>
concept PacketType = requires(P packet)
 {
  Ground<PacketHeader *>( GetPacketHeader(packet) );
 } ;
...
class PacketCanceller : NoCopy
 {
  ...
  public:
   ...
   explicit PacketCanceller(PacketType auto packet_) : packet(GetPacketHeader(packet_)) {}
...
class PacketList : NoCopy
 {
  ...
  public:
   ...
   void put(PacketType auto packet) { put(GetPacketHeader(packet)); }

   void put_first(PacketType auto packet) { put_first(GetPacketHeader(packet)); }
   ...
   void del(PacketType auto packet) { del(GetPacketHeader(packet)); }
