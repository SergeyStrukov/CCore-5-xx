...
void ClientDevice::open(PacketSet<uint8> &pset,StrLen name,const Cfg &cfg)
 {
  ...
  if( !packet )
  ...
  else
    {
     ...
     packet2.pushCompleteFunction(result.functionOf<&OpenResult::complete>());
...
void ClientDevice::close(PacketSet<uint8> &pset,MSec timeout)
 {
  ...
  if( !packet )
  ...
  else
    {
     ...
     packet2.pushCompleteFunction(result.functionOf<&CloseResult::complete>());
...
void ClientDevice::start_write(Packet<uint8> packet)
 {
  ...
  packet2.pushCompleteFunction(functionOf<&ClientDevice::complete_write>());
  ...
  if( getReadInput(input) )
    {
     ...
     packet.pushCompleteFunction(functionOf<&ClientDevice::complete_read>());
...
void ClientDevice::start_read(InputProc *proc)
 {
  ...
  packet2.pushCompleteFunction(functionOf<&ClientDevice::complete_read>());
