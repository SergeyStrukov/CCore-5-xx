...
void NetEvent::Register(EventMetaInfo &info,EventMetaInfo::EventDesc &desc)
 {
  auto id_IPEvent=info.addEnum_uint8("IPEvent"_str)

                      .addValueName(IPTx_ARP,"IP Tx ARP"_str,EventMarker_Up)
                      .addValueName(IPTx_IP,"IP Tx IP"_str,EventMarker_Up)
                      .addValueName(IPTx_UDP,"IP Tx UDP"_str,EventMarker_Up)

                      .addValueName(IPRx_ARP,"IP Rx ARP"_str,EventMarker_Down)
                      .addValueName(IPRx_IP,"IP Rx IP"_str,EventMarker_Down)
                      .addValueName(IPRx_UDP,"IP Rx UDP"_str,EventMarker_Down)

                      .addValueName(IPTx_BadPacketLen,"IP Tx bad packet len"_str,EventMarker_UpBlock)
                      .addValueName(IPTx_TimeoutDrop,"IP Tx timeout drop"_str,EventMarker_UpBlock)
                      .addValueName(IPTx_ARP_Missing,"IP Tx ARP missing"_str,EventMarker_UpBlock)
                      .addValueName(IPTx_ARP_Drop,"IP Tx ARP drop"_str,EventMarker_UpBlock)
                      .addValueName(IPTx_Drop,"IP Tx drop"_str,EventMarker_UpBlock)

                      .addValueName(IPRx_BadPacket,"IP Rx bad packet"_str,EventMarker_DownBlock)
                      .addValueName(IPRx_Drop,"IP Rx drop"_str,EventMarker_DownBlock)
                      .addValueName(IPRx_UDP_NoPort,"IP Rx UDP no port"_str,EventMarker_DownBlock)

                      .getId();

  auto id=info.addStruct("NetEvent"_str)
              .addField_uint32("time"_str,Offset_time)
              .addField_uint16("id"_str,Offset_id)
              .addField_enum_uint8(id_IPEvent,"ip_event"_str,Offset_ip_event)
              .getId();
