...
EventIdType ServerEventRegType::Register(EventMetaInfo &info)
 {
  return info.addEnum_uint8("PTPServer"_str)

             .addValueName(ServerEvent_Trans,"Trans"_str,EventMarker_Push)
             .addValueName(ServerEvent_TransDone,"Trans done"_str,EventMarker_Stop)

             .addValueName(ServerEvent_Timeout,"Timeout"_str,EventMarker_Pass)
             .addValueName(ServerEvent_Cancel,"Cancel"_str,EventMarker_Pass)
             .addValueName(ServerEvent_ClientCancel,"Client cancel"_str,EventMarker_Pass)
             .addValueName(ServerEvent_Abort,"Abort"_str,EventMarker_Pass)

             .addValueName(ServerEvent_CALL,"<- CALL"_str,EventMarker_Down)
             .addValueName(ServerEvent_RECALL,"<- RECALL"_str,EventMarker_Down)
             .addValueName(ServerEvent_SENDRET,"<- SENDRET"_str,EventMarker_Down)
             .addValueName(ServerEvent_ACK,"<- ACK"_str,EventMarker_Down)

             .addValueName(ServerEvent_RET,"-> RET"_str,EventMarker_Up)
             .addValueName(ServerEvent_CANCEL,"-> CANCEL"_str,EventMarker_Up)
             .addValueName(ServerEvent_NOINFO,"-> NOINFO"_str,EventMarker_Up)
             .addValueName(ServerEvent_RERET,"-> RERET"_str,EventMarker_Up)

             .addValueName(ServerEvent_BadTransId,"Bad TransId"_str,EventMarker_Error)

             .addValueName(ServerEvent_NoSlot,"No slot"_str,EventMarker_Error)
             .addValueName(ServerEvent_NoSlotCancel,"No slot cancel"_str,EventMarker_Error)
             .addValueName(ServerEvent_NoFreeSlot,"No free slot"_str,EventMarker_DownBlock)
             .addValueName(ServerEvent_NoPacket,"No packet"_str,EventMarker_Error)

             .addValueName(ServerEvent_BadInbound,"Bad inbound"_str,EventMarker_Error)
             .addValueName(ServerEvent_BadOutbound,"Bad outbound"_str,EventMarker_Error)
...
void ServerProtoEvent::Register(EventMetaInfo &info,EventMetaInfo::EventDesc &desc)
 {
  auto id=info.addStruct("PTPServerEvent"_str)
              .addField_uint32<&ServerProtoEvent::time>("time"_str)
              .addField_uint16<&ServerProtoEvent::id>("id"_str)
              .addField_enum_uint8<&ServerProtoEvent::ev>(EventTypeId<ServerEventRegType>::GetId(),"event"_str)
              .getId();
...
void ServerProtoEvent_slot::Register(EventMetaInfo &info,EventMetaInfo::EventDesc &desc)
 {
  auto id=info.addStruct("PTPServerEventSlot"_str)
              .addField_uint32<&ServerProtoEvent_slot::time>("time"_str)
              .addField_uint16<&ServerProtoEvent_slot::id>("id"_str)
              .addField_enum_uint8<&ServerProtoEvent_slot::ev>(EventTypeId<ServerEventRegType>::GetId(),"event"_str)
              .addField_uint32<&ServerProtoEvent_slot::slot>("slot"_str)
              .getId();
...
void ServerEngine::send(PacketList &complete_list,XPoint point,Packet<uint8> data_packet)
 {
  data_packet.pushCompleteFunction(functionOf<&ServerEngine::do_send>());
