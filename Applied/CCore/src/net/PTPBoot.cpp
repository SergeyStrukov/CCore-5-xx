...
IndexType BootClient::alloc(AddressType address,AddressType len)
 {
  ...
  if( !packet )
  ...
  else
    {
     ...
     packet2.pushCompleteFunction(functionOf<&BootClient::complete_alloc>());
  ...
  while( +data )
    {
     ...
     packet2.pushCompleteFunction(functionOf<&BootClient::complete_write>());
...
void BootClient::boot(AddressType entry_point,FlagType flags)
 {
  ...
  if( !packet )
  ...
  else
    {
     ...
     packet2.pushCompleteFunction(functionOf<&BootClient::complete_boot>());
