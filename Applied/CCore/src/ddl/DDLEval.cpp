...
using Gate = StepEvalGate<EvalContext> ;
...
struct EvalContext::JoinLoopStep
 {
  ...
  JoinLoopStep(PtrLen<Rec> &ret_,PtrLen<Rec> a_,PtrLen<Rec> b_,Rec *out_) : ret(ret_),a(a_),b(b_),cmp(CmpEqual),out(out_) {}
...
struct EvalContext::CmpToOkStep
 {
  ...
  explicit CmpToOkStep(bool &ret_) : ret(ret_),cmp(CmpEqual) {}
...
struct EvalContext::CheckTypeListStep
 {
  ...
  CheckTypeListStep(bool &ret_,TypeListPtr ptr_,TypeListPtr lim_,TypeNode *type_) : ret(ret_),ptr(ptr_),lim(lim_),type(type_),cmp(CmpEqual) {}
...
struct EvalContext::ArrayLenStep
 {
  ...
  ArrayLenStep(Value *ret_,TypeNode *type_,From from_,LenRec &rec_,ExprList &list_,const PosName &ops_)
...
struct EvalContext::CompleteAddPtrStep
 {
  ...
  CompleteAddPtrStep(Value *ret_,Ptr ptr_,SLen delta_,const PosName &ops_)
   : ret(ret_),ptr(ptr_),delta(delta_),ok(false),ops(ops_) {}
...
struct EvalContext::DecayStep
 {
  ...
  DecayStep(Value *ret_,Ptr ptr_,TypeNode *type_,const PosName &ops_) : ret(ret_),ptr(ptr_),type(type_),ok(false),ops(ops_) {}
...
struct EvalContext::CompleteCheckPtrStep
 {
  ...
  CompleteCheckPtrStep(Value *ret_,Ptr ptr_,const PosName &ops_) : ret(ret_),ptr(ptr_),ok(false),ops(ops_) {}
...
struct EvalContext::NeedLockFunc
 {
  ...
  void operator () (unsigned level,TypeNode::Array *ptr) { ret=NeedLock(ptr->type_node,level); }

  void operator () (unsigned level,TypeNode::ArrayLen *ptr) { ret=NeedLock(ptr->type_node,level); }

  void operator () (unsigned level,StructNode *struct_node)
   {
    for(FieldNode &field : struct_node->field_list )
      {
       if( NeedLock(field.type_node,level) )
         {
          ret=true;
...
bool EvalContext::NeedLock(TypeNode *type,unsigned level)
 {
  ...
  ElaborateAnyPtr(func,level-1,TypeAdapter(type).ptr);
...
AnyPtr<TypeNode,TypeList> EvalContext::GetPointerElemType(TypeNode *ptr_type_expected)
 {
  PtrTypeFunc func;
...
TypeNode * EvalContext::GetArrayElemType(TypeNode *array_type_expected)
 {
  ArrayTypeFunc func;
...
EvalContext::DecayCase::DecayCase(TypeNode *ptr_type_expected,TypeNode *array_type_expected)
 {
  type=GetArrayElemType(array_type_expected);

  if( type )
    {
     type_set=GetPointerElemType(ptr_type_expected);
...
EvalContext::EvalContext(ErrorMsg &error_,ElementPool &pool_)
   ...
   int_type{}
 {
  for(int t=TypeNode::Base::TypeIntMin; t<=TypeNode::Base::TypeIntMax ;t++)
    {
     TypeNode *type=pool_.create<TypeNode>();
